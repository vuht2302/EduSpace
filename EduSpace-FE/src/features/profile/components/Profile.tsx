        ...
        <div className="md:col-span-8 bg-surface-container-lowest rounded-xl p-8 flex flex-col md:flex-row items-center md:items-start gap-8 relative overflow-hidden">
          ...
          <div className="flex-1 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center gap-3 mb-2">
              ...
              <span className="px-3 py-1 bg-secondary-container text-on-secondary-container text-[10px] font-bold uppercase tracking-widest rounded-full self-center">
                Top Contributor
              </span>
            </div>
            <p className="text-on-surface-variant mb-6 font-medium">
              Computer Science Major • Stanford University '25
            </p>
      ...
      <section className="mb-12">
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 hide-scrollbar">
          {badges.map((badge) => (
            <div
              key={badge.name}
              className="flex-shrink-0 w-36 h-44 bg-surface-container-lowest rounded-xl flex flex-col items-center justify-center text-center p-4 border border-outline-variant/10 hover:border-primary/30 transition-all"
            >
              <div
                className={`w-16 h-16 rounded-full ${badge.color} flex items-center justify-center mb-3 shadow-sm`}
              >
              ...
              <span className="text-[10px] text-outline mt-1 uppercase">
                {badge.tier}
              </span>
            </div>
          ))}
      ...
      <section className="mt-8">
        <div className="flex gap-8 mb-8 border-b border-outline-variant/20">
          <button className="pb-4 text-primary font-bold border-b-2 border-primary transition-all flex items-center gap-2">
            My Questions{" "}
            <span className="px-2 py-0.5 bg-primary-fixed text-primary text-[10px] rounded-full">
              142
            </span>
          </button>
          <button className="pb-4 text-outline hover:text-on-surface transition-all flex items-center gap-2">
            My Answers{" "}
            <span className="px-2 py-0.5 bg-surface-container-high text-outline text-[10px] rounded-full">
              589
            </span>
          </button>
        </div>
        ...
        <div className="space-y-4">
          {activities.map((activity) => (
            <article
              key={activity.id}
              className={`bg-surface-container-lowest p-6 rounded-xl hover:bg-surface-bright transition-all group border-l-4 ${activity.isPrimary ? "border-primary" : "border-outline-variant/30"} shadow-sm`}
            >
              <div className="flex justify-between items-start mb-4">
                <div className="flex gap-2">
                  {activity.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-1 bg-secondary-container text-on-secondary-container text-[10px] font-bold uppercase tracking-wider rounded-full"
                    >
                      {tag}
                    </span>
                  ))}
