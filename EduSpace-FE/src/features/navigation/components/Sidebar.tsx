...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const trending = [
    {
      tag: "#QuantumDynamics",
      count: t("sidebar.trending.activeDiscussions", { count: 42 }),
    },
    {
      tag: "#CalculusReview",
      count: t("sidebar.trending.activeDiscussions", { count: 128 }),
    },
    {
      tag: "#MidtermPrep",
      count: t("sidebar.trending.activeDiscussions", { count: 310 }),
    },
  ];
  ...
  return (
    <aside
      id="left-sidebar"
      className="hidden lg:flex flex-col w-64 fixed h-[calc(100vh-64px)] overflow-y-auto pt-8 pb-12 px-4 border-r-0"
    >
      <nav className="flex flex-col gap-1 mb-10">
        <span className="text-[10px] uppercase tracking-widest text-outline font-bold mb-4 px-4">
          {t("sidebar.headers.navigation")}
        </span>
        {navItems.map((item) => (
          <button
            key={item.id}
          ...
          >
            <item.icon
              className={`w-5 h-5 ${currentPage === item.id ? "fill-primary/10" : ""}`}
            />
      ...
      <div className="flex flex-col gap-4 mb-10">
        <span className="text-[10px] uppercase tracking-widest text-outline font-bold px-4">
          {t("sidebar.headers.subjectTags")}
        </span>
      ...
      <div className="flex flex-col gap-4">
        <span className="text-[10px] uppercase tracking-widest text-outline font-bold px-4">
          {t("sidebar.headers.trendingTopics")}
        </span>
