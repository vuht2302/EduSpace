...
import { Verified, Trophy, Award, Star } from "../../../types/index.ts";
...
export const Leaderboard: React.FC = () => {
  ...
  const others = [
    ...
    },
    {
      ...
      rep: "9,440",
      status: "Contributor",
      avatar: "https://picsum.photos/seed/marcusw/100/100",
    },
    {
      rank: "07",
      name: "Zara Mitchell",
    ...
    <div id="leaderboard-view" className="max-w-6xl mx-auto w-full">
      <header className="mb-12">
        ...
        <p className="text-on-surface-variant text-lg max-w-2xl">
          Recognizing the most impactful contributors in the ScholarFlow
          academic community. Reputation is earned through verified answers and
          peer insights.
        </p>
      </header>
        ...
        {topThree.map((scholar) => (
          ...
          >
            ...
            <div className="relative mb-6">
              <div
                className={`w-24 h-24 rounded-full p-1 ${scholar.color} ${scholar.isTop ? "w-28 h-28" : ""}`}
              >
                <img
                  src={scholar.avatar}
                  alt={scholar.name}
              ...
              <div
                className={`absolute -bottom-2 left-1/2 -translate-x-1/2 text-xs font-bold px-3 py-1 rounded-full shadow-md ${scholar.isTop ? "bg-secondary-container text-on-secondary-container px-4 py-1.5" : "bg-slate-400 text-white"}`}
              >
                {scholar.badge}
              </div>
            </div>
            <h3
              className={`text-xl font-bold mb-1 ${scholar.isTop ? "text-2xl" : ""}`}
            >
              {scholar.name}
            </h3>
            <p
              className={`font-semibold text-lg ${scholar.isTop ? "text-primary-fixed" : "text-primary"}`}
            >
              {scholar.reputation} Reputation
            </p>
            <div className="mt-4 flex gap-2">
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${scholar.isTop ? "bg-white/20" : "bg-secondary-container text-on-secondary-container"}`}
              >
                {scholar.tag}
              </span>
              {scholar.isVerified && (
                <span
                  className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase ${scholar.isTop ? "bg-white/20" : "bg-tertiary-container text-on-tertiary-container"}`}
                >
                  Verified
                </span>
              )}
      ...
      <section className="space-y-4">
        ...
        {others.map((scholar) => (
          <div
            key={scholar.rank}
            className="bg-surface-container-lowest hover:bg-surface-bright p-5 rounded-xl transition-all flex items-center justify-between group cursor-pointer shadow-sm"
          >
            <div className="flex items-center gap-6">
              <span className="text-xl font-black text-outline/30 w-8 group-hover:text-primary transition-colors">
                {scholar.rank}
              </span>
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-on-surface-variant flex items-center gap-1">
                    {scholar.sub}
                    {scholar.isVerified && (
                      <Verified className="w-3 h-3 text-tertiary-container fill-current" />
                    )}
                  </p>
                </div>
              </div>
