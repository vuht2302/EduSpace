      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-outline font-medium">
              {question.timestamp} • {question.author.grade}
            </p>
          </div>
        </div>
        <div className="flex gap-2">
          {question.tags.slice(0, 1).map((tag) => (
            <span
              key={tag}
              className="px-3 py-1 rounded-full bg-secondary-container/30 text-on-secondary-container text-[10px] font-bold uppercase tracking-wider"
            >
              {tag}
            </span>
          ))}
